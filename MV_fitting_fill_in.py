...
p0 = [FILL IN HERE] # make some tau guesses

# fit to our model
popt, pcov = curve_fit(FUNCTION, FILL IN, FILL IN, p0=FILL IN)

print("Fitted taus:", FILL IN) # let's print what our taus were
...
plt.plot(t, FILL IN, label="EF fit")

plt.plot(t, ER_data, alpha=0.3, label="ER data")
plt.plot(t, FILL IN, label="ER fit")

plt.plot(t, GS_data, alpha=0.3, label="GS data")
plt.plot(t, FILL IN, label="GS fit")

plt.legend()
plt.show()

# when you are done, feel free to go into datagenerator.py and try other tau values, noise levels, etc. 
