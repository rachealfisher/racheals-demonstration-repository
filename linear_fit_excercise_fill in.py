...
# it looks like a line! let's try a linear function.
...
popt, pcov = curve_fit(FILL IN, FILL IN, FILL IN, p0= FILL IN ) # curve_fit(function, x, y, guesses)
m_fit, b_fit = popt # popt = ?
print(f"fitted m, b = {m_fit}, {b_fit}") # print your outputs if you want

y_fitted = line_function(FILL IN)

# let's plot our fake data again with our fit over it
plt.figure()
plt.scatter(x,y_noisy, label= "data")
plt.plot(FILL IN, FILL IN, label="fit")
plt.legend()
plt.show()

# if you have finished, try playing around with the noise level/guesses.
# how does increasing the noise level affect the accuracy of your fitted parameters?
# what happens if you try to write another function? Say, a quadratic function?
